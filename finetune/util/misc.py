...
def save_model(
    ...
    exceptions = ["log.txt"]
    file_names = [
        file for file in file_names if file not in exceptions and "epoch" in file
    ]
    ...
    if len(file_names) >= nb_ckpts_max and is_main_process():
        file_names = sorted(
            file_names,
            key=lambda x: float(x.split(".pth")[0].split("-")[-1]),
            reverse=True,
        )
    ...
    if loss_scaler is not None:
        if last:
            checkpoint_paths = [output_dir / (f"last.pth")]
        else:
...
def save_best_model(
    ...
    file_names = [
        file
        for file in file_names
        if file not in exceptions and evaluation_criterion in file
    ]
    ...
    if len(file_names) >= nb_ckpts_max and is_main_process():
        ...
        if mode == "increasing":
            ...
            file_names = sorted(
                file_names, key=lambda x: float(x.split(".pth")[0].split("-")[-1])
            )
        os.remove(os.path.join(output_dir, file_names[-1]))
    ...
    if loss_scaler is not None:
        checkpoint_paths = [
            output_dir
            / (
                f"checkpoint-{epoch_name}-{evaluation_criterion}-{test_stats[evaluation_criterion]:.4f}.pth"
            )
        ]
        for checkpoint_path in checkpoint_paths:
            to_save = {
...
def get_best_ckpt(data_path, eval_criterion):
    print(
        f"Searching for the checkpoint file with the best evaluation score in: {data_path}"
    )
...
def add_weight_decay_timm(model, weight_decay=1e-5, skip_list=()):
    decay = []
    no_decay = []
    for name, param in model.named_parameters():
        if not param.requires_grad:
