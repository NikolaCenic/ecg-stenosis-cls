    ...
    if "pos_embed_x" in checkpoint_model:
        ...
        if num_tokens_model > num_tokens_ckpt:
            print(
                ...
                % (1, num_tokens_ckpt, 1, num_tokens_model)
            )
            # (1, T'_max, D/2)
            ...
            model.pos_embed_x = torch.nn.Parameter(
                torch.zeros(1, num_tokens_model + 1, embedding_size),
                requires_grad=False,
            )
        else:
            model.pos_embed_x = torch.nn.Parameter(
                torch.zeros(1, num_tokens_ckpt + 1, embedding_size), requires_grad=False
            )
...
def interpolate_decoder_pos_embed_x(model, checkpoint_model):
    if "decoder_pos_embed_x" in checkpoint_model:
        ...
        if num_tokens_model > num_tokens_ckpt:
            ...
            model.decoder_pos_embed_x = torch.nn.Parameter(
                torch.zeros(1, num_tokens_model + 1, embedding_size),
                requires_grad=False,
            )
        else:
