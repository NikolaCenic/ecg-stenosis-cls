...
class MeanSquaredError:
    ...
    def _squared_sum(self, y_pred, y) -> list:
        ...
        if len(self.running_squared_sum) == 0:
            ...
            return [
                sum(squared_sum_values)
                for squared_sum_values in zip(squared_sum, self.running_squared_sum)
            ]
    ...
    def __call__(self, y_pred, y) -> list:
        ...
        return [
            (rss / self.N) if self.squared else (rss / self.N) ** 0.5
            for rss in self.running_squared_sum
        ]
...
def update_best_stats(
    best_stats: List, test_stat: float, best_stats_len: int, mode="increasing"
) -> List:
    if mode == "decreasing":
        # e.g. loss
