...
class Rescaling(object):
    ...
    def __call__(self, sample) -> Any:
        return sample * torch.normal(
            mean=torch.Tensor([1]), std=torch.Tensor([self.sigma])
        )
...
class Jitter(object):
    ...
    def __call__(self, sample) -> Any:
        ...
        return sample + amplitude * torch.normal(
            mean=0, std=self.sigma, size=sample.shape
        )
            ...
            if sample_dims == 2:
                ...
                for bin in range(X_f.shape[0]):
                    ...
                    for ch in range(X_f.shape[1]):
                        ...
                        # concatenate the real and imaginary parts
                        complex_pair = torch.cat(
                            (real_part, imag_part), dim=0
                        )  # .unsqueeze(dim=0)

                        # concatenate the channels
                        X_f_bin_complex = torch.cat(
                            (X_f_bin_complex, complex_pair), dim=0
                        )

                    # concatenate the frequency bins
                    X_f_complex = torch.cat(
                        (X_f_complex, X_f_bin_complex.unsqueeze(dim=0)), dim=0
                    )
        else:
            # if you want real and imag part to be concatenated
...
class CropResizing(object):
    ...
    def __call__(self, sample) -> torch.Tensor:
        ...
        if self.fixed_crop_len is None:
            crop_len = int(
                max_nb_time_steps
                * np.random.uniform(low=self.lower_bnd, high=self.upper_bnd)
            )
        ...
        if self.resize:
            ...
            if (
                self.fixed_resize_len is None
                or self.fixed_resize_len > max_nb_time_steps
            ):
            ...
            if cropped_sample.dim() > 3:
                ...
                cropped_sample = torch.nn.functional.interpolate(
                    cropped_sample, size=resize_len, mode="linear"
                )
        ...
        if np.random.uniform() < self.prob:
            ...
            if sample_dims == 2:
                for ch in range(sample.shape[-2]):
                    sample_interpolated[ch] = np.interp(
                        np.arange(0, sample.shape[-1]),
                        np.arange(0, sample.shape[-1], step=self.step),
                        sample_sub[ch],
                    )
            elif sample_dims == 3:
                for f_bin in range(sample.shape[-3]):
                    for ch in range(sample.shape[-2]):
                        sample_interpolated[f_bin, ch] = np.interp(
                            np.arange(0, sample.shape[-1]),
                            np.arange(0, sample.shape[-1], step=self.step),
                            sample_sub[f_bin, ch],
                        )
...
class Masking(object):
    ...
    def __init__(
        ...
        self.patch_size = int(
            patch_size_sec * fs
        )  # patch_size[ticks] = patch_size[sec] * fs[Hz]
    ...
    def __call__(self, sample) -> Any:
        if np.random.uniform() < self.prob:
            ...
            ones_sizes = np.round(
                indices_weights[:, :nb_patches].T
                * (number_of_ones / np.sum(indices_weights, axis=-1))
            ).T.astype(np.int32)
...
class FTSurrogate(object):
    def __init__(
        self, phase_noise_magnitude, channel_indep=False, seed=None, prob=1.0
    ) -> None:
    ...
    def _new_random_fft_phase_odd(self, c, n, device="cpu", seed=None):
        ...
        random_phase = torch.from_numpy(2j * np.pi * rng.random((c, (n - 1) // 2))).to(
            device
        )
    ...
    def _new_random_fft_phase_even(self, c, n, device="cpu", seed=None):
        rng = check_random_state(seed)
        random_phase = torch.from_numpy(2j * np.pi * rng.random((c, n // 2 - 1))).to(
            device
        )
    ...
    def __call__(self, sample) -> Any:
        if np.random.uniform() < self.prob:
            ...
            if isinstance(self.phase_noise_magnitude, torch.Tensor):
                self.phase_noise_magnitude = self.phase_noise_magnitude.to(
                    sample.device
                )
...
class SpecAugment(object):
        ...
        if sample_dim < 3:
            ...
            for i in range(sample.shape[0]):
                ...
                masked_sample = torch.cat(
                    (masked_sample, masked_sub_sample.unsqueeze(0)), dim=0
                )
        else:
    ...
    def _mask_spectrogram(self, sample):
        ...
        if random.random() < 0.5:
            ...
            # mask the bins
            spec[..., start_idx:end_idx, :] = 0.0 + 0.0j
        else:
            # time domain
...
class FinetuneECGTransforms:
    ...
    # Time shift (roll)
    @staticmethod
    def time_shift(x, fs, p=0.4, shift_max_sec=1):
        if random.random() > p:
            return x
        shift = int(random.uniform(-shift_max_sec, shift_max_sec) * fs)
        return torch.roll(x, shifts=shift, dims=-1)

    # Random dropout of small segments
    @staticmethod
    def drop_segments(x, fs, p=0.5, segment_max_sec=0.3, drop_cnt=5):
        if random.random() > p:
            return x
        mask = torch.ones_like(x)
        length = x.shape[-1]
        seg_len = int(segment_max_sec * fs)
    ...
    def lowpass(self, x, p=0.1):
        if random.random() > p:
            return x
        kernel_size = int(0.1 * self.fs)  # 100ms window
    ...
    def __call__(self, x):
        x = FinetuneECGTransforms.time_shift(x, fs=self.fs)
        x = FinetuneECGTransforms.scale(x)
        x = FinetuneECGTransforms.add_noise(x)
        x = FinetuneECGTransforms.drop_segments(x, fs=self.fs)
        return x
