...
def norm(data: torch.Tensor, attn_mask: torch.Tensor = None) -> torch.Tensor:
    ...
    if attn_mask is None:
        ...
        mean = torch.sum(data, dim=-1, keepdim=True) / (
            torch.sum(attn_mask, dim=-1, keepdim=True) + 1e-9
        )
        ...
        var = torch.sum((data - mean + aux_mask) ** 2, dim=-1, keepdim=True) / (
            torch.sum(attn_mask, dim=-1, keepdim=True) + 1e-9
        )
...
def ncc(
    ...
    if attn_mask is None:
        ...
        # (B, C, H)
        cross_corrs = torch.sum(norm(data=data_0) * norm(data=data_1), dim=-1) / (
            data_0.shape[-1] + 1e-9
        )
    else:
        ...
        # (B, C, H)
        cross_corrs = torch.sum(
            norm(data=data_0, attn_mask=attn_mask)
            * norm(data=data_1, attn_mask=attn_mask),
            dim=-1,
        ) / (torch.sum(attn_mask, dim=-1) + 1e-9)
...
def masked_mean(tensor, attn_mask=None, dim=None, keep_dim=False):
    ...
    if attn_mask is None:
        ...
        if dim is None:
            return torch.sum(tensor_masked) / (
                torch.sum(attn_mask.unsqueeze(-1).repeat(1, 1, tensor.shape[-1])) + 1e-9
            )
        else:
            numerator = torch.sum(tensor_masked, dim=dim, keepdim=keep_dim)
            denominator = (
                torch.sum(
                    attn_mask.unsqueeze(-1).repeat(1, 1, tensor.shape[-1]),
                    dim=dim,
                    keepdim=keep_dim,
                )
                + 1e-9
...
def masked_var(tensor, attn_mask=None, dim=None, keep_dim=False):
    ...
    if attn_mask is None:
        ...
        tensor_masked = tensor * attn_mask.unsqueeze(-1)
        deviations = tensor_masked - masked_mean(
            tensor, attn_mask, dim=dim, keep_dim=True
        )
        deviations = deviations * attn_mask.unsqueeze(-1)
        squared_deviations = (deviations**2) * attn_mask.unsqueeze(-1)
        if dim is None:
            return torch.sum(squared_deviations) / (
                torch.sum(attn_mask.unsqueeze(-1).repeat(1, 1, tensor.shape[-1]))
                - 1
                + 1e-9
            )
        else:
            return torch.sum(squared_deviations, dim=dim, keepdim=keep_dim) / (
                torch.sum(
                    attn_mask.unsqueeze(-1).repeat(1, 1, tensor.shape[-1]),
                    dim=dim,
                    keepdim=keep_dim,
