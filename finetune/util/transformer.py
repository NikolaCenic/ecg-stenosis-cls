...
class Attention(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        q, k, v = qkv.unbind(
            0
        )  # make torchscript happy (cannot use tensor as tuple) (B, N, D)
...
class TemporalRoPEAttention(nn.Module):
    ...
    def __init__(
        self, dim, num_heads=8, T_max=1, qkv_bias=False, attn_drop=0.0, proj_drop=0.0
    ):
        super().__init__()
        assert dim % num_heads == 0, "dim should be divisible by num_heads"
        self.num_heads = num_heads
    ...
    def forward(
        ...
        if attn_mask_input is not None:
            ...
            self.mask_ratio = (
                1 - (attn_mask.shape[-1] - 1) / ids_restore.shape[-1]
            )  # the cls token is the 1 subtracted
            # print("N:", N, "V:", V, "T:", T, "mask_ratio:", self.mask_ratio)
            expected_N = round(V * T * (1 - self.mask_ratio), 0) + 1
            # print("Expected N:", expected_N)
            assert (
                N == expected_N
            ), f"Sequence length N ({N}) must match V * T * (1 - mask_ratio) + 1 cls token ({expected_N})."
        else:
            V, T = V_input, T_input
        ...
        q, k, v = qkv.unbind(
            0
        )  # make torchscript happy (cannot use tensor as tuple) (B, 1+N_v, D)
    ...
    def apply_rope_to_time(self, q, k, V, ids_restore=None):
        ...
        if self.compute_rope_embeddings == True:
            ...
            theta = 10000.0 ** (
                -torch.arange(0, self.head_dim, 2, device=device).type_as(q)
                / self.head_dim
            )  # (head_dim // 2)
            position = torch.arange(1 + self.T_max, device=device).type_as(
                q
            )  # (1+T_max)
            angles = torch.einsum(
                "t,d->td", position, theta
            )  # (1+T_max, head_dim // 2)
        ...
        def rotate(x, ids_restore):
            ...
            ids_restore: (B, N)
            """
            x_rot_cls = x[:, :1].view(B, 1, -1, self.head_dim)[
                ..., ::2
            ]  # Select even indices, (B,    1, nb_heads, head_dim // 2)
            x_im_cls = x[:, :1].view(B, 1, -1, self.head_dim)[
                ..., 1::2
            ]  # Select odd indices,  (B,    1, nb_heads, head_dim // 2)

            x_cls_new = torch.cat(
                [
                    x_rot_cls * self.cos[:, :1] - x_im_cls * self.sin[:, :1],
                    x_rot_cls * self.sin[:, :1] + x_im_cls * self.cos[:, :1],
                ],
                dim=-1,
            )  # (B, 1, nb_heads, head_dim)

            x_rot = x[:, 1:].view(B, N - 1, -1, self.head_dim)[
                ..., ::2
            ]  # Select even indices, (B,  N_v, nb_heads, head_dim // 2)
            x_im = x[:, 1:].view(B, N - 1, -1, self.head_dim)[
                ..., 1::2
            ]  # Select odd indices,  (B,  N_v, nb_heads, head_dim // 2)

            cos = self.cos[:, 1:].expand(B, -1, -1, -1)  # (B, T_max, 1, head_dim // 2)
            sin = self.sin[:, 1:].expand(B, -1, -1, -1)  # (B, T_max, 1, head_dim // 2)
            if ids_restore is not None:
                ids_shuffle = (
                    torch.argsort(ids_restore, dim=1)[:, : N - 1] % V
                )  # (B, N_v)
            else:
                ids_shuffle = (
                    torch.arange(start=0, end=N - 1, device=device)
                    .unsqueeze(0)
                    .expand(B, -1)
                    % V
                )  # (B, N_v)
            cos = torch.gather(
                cos, 1, ids_shuffle[..., None, None].repeat(1, 1, 1, self.head_dim // 2)
            )  # (B, N_v, 1, head_dim // 2)
            sin = torch.gather(
                sin, 1, ids_shuffle[..., None, None].repeat(1, 1, 1, self.head_dim // 2)
            )  # (B, N_v, 1, head_dim // 2)

            x_new = torch.cat(
                [x_rot * cos - x_im * sin, x_rot * sin + x_im * cos], dim=-1
            )  # (B,   N_v, nb_heads, head_dim)

            x_new = torch.cat(
                [x_cls_new, x_new], dim=1
            )  # (B, 1+N_v, nb_heads, head_dim)
