    ...
    def __call__(self, sample) -> torch.Tensor:
        if self.mode == "sample_wise":
            ...
            for idx in self.groups:
                mean_group = torch.mean(
                    sample[lower_bound:idx], dim=(0, 1), keepdim=True
                )
        ...
        normalized_sample = (sample - mean) / (var + 1.0e-9) ** 0.5
...
class MinMaxScaling(object):
    ...
    def __call__(self, sample) -> torch.Tensor:
        ...
        rescaled_sample = (sample - min) / (max - min) * (
...
class IdealFiltering(object):
    ...
    def __init__(
        self, fs: int = 250, f_0: int = 100, band_width: int = 5, mode: str = "low_pass"
    ) -> None:
        super().__init__()
        self.fs = fs
        self.f_0 = f_0
    ...
    def __call__(self, sample) -> torch.Tensor:
        ...
        X_f = (
            1 / N * fft.fftshift(fft.fft(sample, n=N))
        )  # Factor 1/N is not crucial, only for visualization
...
class ButterworthFiltering(object):
    ...
    def __call__(self, sample) -> torch.Tensor:
        ...
        if self.mode == "low_pass":
            sos = signal.butter(
                self.order, self.f_0, "lowpass", output="sos", fs=self.fs
            )
        elif self.mode == "high_pass":
            sos = signal.butter(
                self.order, self.f_0, "highpass", output="sos", fs=self.fs
            )
        elif self.mode == "band_stop":
            sos = signal.butter(
                self.order,
                [self.f_0 - (self.band_width / 2), self.f_0 + (self.band_width / 2)],
                "bandstop",
                output="sos",
                fs=self.fs,
            )
...
class GaussianFiltering(object):
    ...
    def __call__(self, sample) -> torch.Tensor:
        ...
        if self.mode == "low_pass":
            ...
            std = self.band_width
            Filter = 1 - (
                torch.exp(-((f + self.f_0) / (std)).pow(2))
                + torch.exp(-((f - self.f_0) / (std)).pow(2))
            )
        elif self.mode == "band_pass":
            std = self.band_width
            Filter = torch.exp(-((f + self.f_0) / (std)).pow(2)) + torch.exp(
                -((f - self.f_0) / (std)).pow(2)
            )
        else:
            sys.exit("Error: Mode does not exist.")
