...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.patch_embed = PatchEmbed(
            img_size[0], patch_size, embed_dim, flatten=False
        )  # set flatten to False
        ...
        self.pos_embed_x = nn.Parameter(
            torch.zeros(1, self.max_num_patches_x + 1, embed_dim // 2),
            requires_grad=False,
        )  # +1 cls embed

        total_num_embeddings_y = sum([v for k, v in self.grid_height.items()])
        self.pos_embed_y = nn.Embedding(
            total_num_embeddings_y + 1, embed_dim // 2, padding_idx=0
        )  # +1 padding embed
        ...
        for block in self.blocks:
            ...
            block.attn = Attention(
                kwargs["embed_dim"], kwargs["num_heads"], qkv_bias=kwargs["qkv_bias"]
            )
    ...
    def initialize_weights(self):
        ...
        _pos_embed_y = torch.nn.Parameter(
            torch.randn(
                self.pos_embed_y.num_embeddings - 1, self.pos_embed_y.embedding_dim
            )
            * 0.02
        )
        ...
        _pos_embed_x = get_1d_sincos_pos_embed(
            self.pos_embed_x.shape[-1], self.pos_embed_x.shape[-2] - 1, cls_token=True
        )
    ...
    def freeze_first_n_blocks(self, n):
    ...
    def random_masking(self, x, mask_ratio):
        ...
        len_keep = math.ceil(
            L * (10 - 10 * mask_ratio) / 10
        )  # factor 10 to compensate float precision
    ...
    def random_masking_blockwise(self, x, mask_c_ratio, mask_t_ratio):
        ...
        """
        N, L, D = x.shape  # batch, length, dim
        C, T = int(self.img_size[-2] / self.patch_size[-2]), int(
    ...
    def forward_features(self, x, pos_embed_y):
        ...
        pos_embed_x = torch.nn.functional.pad(
            pos_embed_x, (x.shape[1] // 2, 0), "constant", 0
        )
        ...
        pos_embed_y_batch = torch.nn.functional.pad(
            pos_embed_y_batch, (0, x.shape[1] // 2), "constant", 0
        )
